...
void Initialize() {
    ...
    for (int i = 0; i < PLATFORM_COUNT; i++) state.platforms[i].Update(0, NULL, NULL, 0, NULL, 0);

    state.enemies = new Entity[ENEMY_COUNT];
    GLuint enemyTextureID = LoadTexture("flying bug.png");
    ...
    state.enemies[1].aiType = LURCHER;
    ...
    state.enemies[2].acceleration = glm::vec3(0, -0.1f, 0);
...
void ProcessInput() {
    ...
    if (keys[SDL_SCANCODE_LEFT]) state.player->position.x -= 0.001f;
    else if (keys[SDL_SCANCODE_RIGHT]) state.player->position.x += 0.001f;
...
void Render() {
    ...
    if (missionComplete == false && !(state.player->isActive)) {
        DrawText(&program, LoadTexture("font.png"), "You Lose!",
        ...
        DrawText(&program, LoadTexture("font.png"), "You Win!", 0.5f, -0.2f, glm::vec3(-3.25f, 0.5f, 0));
        state.player->acceleration.x = 0;
        state.player->acceleration.y = 0;
        state.player->velocity.x = 0;
        state.player->velocity.y = 0;
